function [TR, TT, ER,normals,norm_out,I,t] = icp(q,p,varargin)
...
inp.addParamValue('Features', [], @(x)isstruct(x));
...
inp.addParamValue('IDX_out', [], @(x)isreal(x) && size(x,1) == 1);
...
% get new mr cloud
qq      = q';
[B,I]   = unique(qq(:,1:2),'rows');
qq      = [B,qq(I,3)];
if ~isempty(arg.Features)
   arg.Features.q = arg.Features.q(:,I);
end
q       = qq';
...
if (strcmp(arg.Minimize, 'plane') && isempty(arg.Normals))
    ...
    arg.Normals = lsqnormest2(q);
    normals     = arg.Normals;
    
    % check for nans in normals and delete entries
    id_del = find(isnan(normals(1,:)));
    disp([num2str(length(id_del)) ' points deleted due to invalid normal vector!']);
    q(:,id_del) = [];
    arg.Normals(:,id_del) = [];
    normals(:,id_del) = [];
else
    if(~isempty(arg.IDX_out))
        id_del      = arg.IDX_out;
        q(:,id_del) = [];
    else
        id_del = [];
...
% delete entries in Feature if existing
if ~isempty(arg.Features)
    arg.Features.q(:,id_del) = [];
end
...
if str2double(v.Version) >= 8.4
    ...
    KDtree = @KDTreeSearcher_TW;
end

if strcmp(arg.Matching, 'kDtree')
    kdOBJ = KDtree(transpose(q));
    %--TOBIAS--
    if ~isempty(arg.Features)
        kdOBJ = KDtree(transpose([q;arg.Features.q]));
    end
end
...
if arg.EdgeRejection
    if isempty(arg.Boundary)
        ...
        if(exist('id_del','var'))
            [~,b] = intersect(arg.Boundary,id_del);
            arg.Boundary(b) = [];
        end
        bdr = arg.Boundary;
    end
...
for k=1:arg.iter
    ...
    switch arg.Matching
        ...
        case 'kDtree'
            if ~isempty(arg.Features)
                [match mindist] = match_kDtree([q;arg.Features.q],[pt;arg.Features.p],kdOBJ);
            else
                [match mindist] = match_kDtree(q,pt,kdOBJ);
            end
            if(length(unique(match))==1)
                tempmatch = randperm(size(q,2));
                match = tempmatch(1:length(match));
            end
    end
...
[match mindist] = kdOBJ.knnsearch(transpose(p));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% vgl. Chen-Medioni paper und Priceton paper "ICPstability"
function [R,T] = eq_plane(q,p,n,weights)

% Löse C*X = b, wobei b = [alpha, beta, gamma, tx, ty, tz]
...
function [normals] = lsqnormest2(pcld)

% step factor
fac = 0.01;
pval        = pcld';
F           = TriScatteredInterp(pval(:,1),pval(:,2),pval(:,3));

for i = 1:size(pval,1)
    % obtain normal vector as mean from different tangential planes
    p               = [pval(i,1); pval(i,2); pval(i,3)];                                                 % Ortsvektor
    v1              = [pval(i,1) + fac; pval(i,2); F(pval(i,1) + fac,pval(i,2))] - p;                    % Tangentialvektor 1
    v2              = [pval(i,1); pval(i,2) + fac; F(pval(i,1), pval(i,2) + fac)] - p;              % Tangentialvektor 2
    v1              = v1/norm(v1);
    v2              = v2/norm(v2);
    n(1,:)          = cross(v2,v1)/norm(cross(v2,v1));
    ...
    v1              = v1/norm(v1);
    v2              = v2/norm(v2);
    n(2,:)          = -cross(v2,v1)/norm(cross(v2,v1));
    ...
    v1              = v1/norm(v1);
    v2              = v2/norm(v2);
    n(3,:)          = -cross(v2,v1)/norm(cross(v2,v1));
    ...
    v1              = v1/norm(v1);
    v2              = v2/norm(v2);
    n(4,:)          = cross(v2,v1)/norm(cross(v2,v1));
    
    
    normals(i,:)    = mean(n(~isnan(sum(n,2)),:),1);
end
normals = normals';
end
